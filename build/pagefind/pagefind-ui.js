;(() => {
					...
					: (this._observer = new ResizeObserver((t) => {
							...
							for (let r of t)
								fe.entries.set(r.target, r),
									(s = this._listeners.get(r.target)) === null || s === void 0 || s(r)
						}))
			}
		}
		...
		if (l > 1 && l < 9) {
			if (((t.language = s.slice(0, l)), l < 4)) {
				...
				for (
					...
					if (a > 2)
						return i(l, 3, 'Too many extended language subtags, expected at most 3 subtags')
			...
			for (
				...
				t.extensions.push({ singleton: s.charAt(l + 1), extensions: s.slice(l + 3, a).split('-') }),
					(l = a)
			}
		} else l = 0
		if (
					...
					e !==
						(e =
							s[18]('many_results', s[17], s[5])
								...
								.replace(
									/\[COUNT\]/,
									new Intl.NumberFormat(s[5].language).format(s[11].results.length)
								) + '') &&
					N(t, e)
			},
			d(s) {
	...
	function Vn(n) {
		...
		return {
			...
			p(r, l) {
				l[0] & 147488 &&
					t !== (t = r[18]('searching', r[17], r[5]).replace(/\[SEARCH_TERM\]/, r[14]) + '') &&
					N(s, t)
			},
			d(r) {
				r && C(e)
	...
	function vi(n) {
		...
		return {
			...
			p(T, S) {
				;(!E || (S[0] & 131104 && r !== (r = T[18]('placeholder', T[17], T[5])))) &&
					p(s, 'placeholder', r),
					...
					(!E || (S[0] & 131104 && c !== (c = T[18]('search_label', T[17], T[5])))) &&
						p(t, 'aria-label', c),
